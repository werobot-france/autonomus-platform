...
class Robot:
			...
			for pt in self.tp:
				for pr in self.rp:
					for mur in self.murs:
						print('pt', pt)
						print('pr', pr)
						if not self.intersect(pt[-1][0], pt[-1][1], pr[-1][0], pr[-1][1]):
							npt = pt.reverse()
							return pr + npt
			self.expandPaths()
	def expandPaths(self):
		...
		for p in self.tp:
			for mur in self.murs:
						if (not self.intersect(p[-1], [p[-1][0], p[-1][1]+self.precision], mur[0], mur[1])) and ([p[-1][0], p[-1][1]+self.precision] != p[-2]):
							np = []
							for i in p:
								np.append(i)
							np.append([p[-1][0], p[-1][1]+self.precision])
							ntp.append(np)
						if (not self.intersect(p[-1], [p[-1][0]+self.precision, p[-1][1]], mur[0], mur[1])) and ([p[-1][0]+self.precision, p[-1][1]] != p[-2]):
							np = []
							for i in p:
								np.append(i)
